...
- [docs/http-resilience.md](./docs/http-resilience.md) — the circuit breaker,
  bulkhead, retry ladder and deadline in front of every outbound call: why a 404
  must not open a breaker while a 429 must, why a retry is only safe for a
  request carrying an idempotency key (and which of the payment and notification
  calls that leaves un-retried), why the ladder sits outside the breaker rather
  than inside it, why opossum's own timeout is off when `AbortSignal` is already
  on, why `Retry-After` is honoured but still jittered, why the concurrency cap
  is the only thing that sees a dependency which is slow rather than broken, why
